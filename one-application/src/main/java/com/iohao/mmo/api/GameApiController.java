...
public class GameApiController {
    ...
    @Resource
    com.iohao.mmo.story.repository.AnimatedPortraitRepository animatedPortraitRepository;

    @Resource
    com.iohao.mmo.common.ai.video.AiVideoProvider videoProvider;
    ...
    @GetMapping("/animated-portrait/{id}")
    public ResponseEntity<byte[]> getAnimatedPortrait(@PathVariable String id) {
        var opt = animatedPortraitRepository.findById(id);
        if (opt.isEmpty() || opt.get().getAnimationData() == null) {
            return ResponseEntity.notFound().build();
        }
        var ap = opt.get();
        return ResponseEntity.ok()
                ...
                .header("Cache-Control", "max-age=86400")
                .body(ap.getAnimationData());
    }

    @PostMapping("/animated-portrait/{id}/favorite")
    public ResponseEntity<Map<String, Object>> toggleFavorite(@PathVariable String id) {
        var opt = animatedPortraitRepository.findById(id);
        if (opt.isEmpty()) return err("未找到");
        var ap = opt.get();
        ap.setFavorite(!ap.isFavorite());
        animatedPortraitRepository.save(ap);
        return ok(Map.of("favorite", ap.isFavorite()));
    }

    @DeleteMapping("/animated-portrait/{id}")
    public ResponseEntity<Map<String, Object>> deleteAnimatedPortrait(@PathVariable String id) {
        animatedPortraitRepository.deleteById(id);
        return ok(Map.of("deleted", true));
    }

    @PostMapping("/portrait/animate")
    public ResponseEntity<Map<String, Object>> animatePortrait(
            @RequestBody Map<String, Object> body, HttpSession session) {
        String target = (String) body.getOrDefault("target", "person");
        String targetId = (String) body.get("targetId");
        String prompt = (String) body.get("prompt");
        String imageUrl = (String) body.get("imageUrl");
        String videoModel = (String) body.getOrDefault("videoModel", "wan");
        int reqSteps = body.get("steps") instanceof Number n ? n.intValue() : 0;
        int reqFrames = body.get("frames") instanceof Number n ? n.intValue() : 0;

        if (imageUrl == null || imageUrl.isBlank()) return err("缺少 imageUrl");
        byte[] bytes = loadReferenceBytes(imageUrl);
        if (bytes == null) return err("源图加载失败：" + imageUrl);

        String cacheKey = switch (target) {
            case "pet" -> "anim_pet_" + targetId;
            case "companion" -> "anim_comp_" + targetId;
            case "monster" -> "anim_monster_" + targetId;
            default -> "anim_person_" + requireLogin(session);
        };

        if (prompt == null || prompt.isBlank()) {
            prompt = "subtle idle animation, breathing, hair sway, black background";
        }


        int w = 768, h = 1024;
        int frames = reqFrames > 0 ? reqFrames : 17;
        int steps = reqSteps > 0 ? reqSteps : 8;
        log.info("动画立绘生成开始 target={} cacheKey={} size={}x{} frames={} steps={} prompt={}", target, cacheKey, w, h, frames, steps, prompt);
        long t0 = System.currentTimeMillis();
        try {
            String sourceRef = videoProvider.prepareSource(bytes);
            var req = com.iohao.mmo.common.ai.video.AiVideoRequest.builder()
                    .prompt(prompt)
                    ...
                    .build();
            var result = videoProvider.generate(req);
            byte[] finalData = result.getData();
            if ("webp".equalsIgnoreCase(result.getExt())) {
                try {
                    finalData = removeBlackBg(finalData);
                } catch (Exception e) {
                    log.warn("动画抠图失败，使用原始输出", e);
                }
            }
            // 存入 MongoDB
            var ap = new com.iohao.mmo.story.entity.AnimatedPortrait();
            ap.setGroupKey(cacheKey);
            ap.setAnimationData(finalData);
            ap.setContentType("image/webp");
            ap.setPrompt(prompt);
            ap.setFrames(result.getFrames());
            ap.setCreateTime(System.currentTimeMillis());
            animatedPortraitRepository.save(ap);
            String url = "/api/animated-portrait/" + ap.getId();
            log.info("动画立绘生成完成 cost={}ms cacheKey={} url={}", System.currentTimeMillis() - t0, cacheKey, url);
            return ok(Map.of("animatedPortraitUrl", url, "frames", result.getFrames()));
        } catch (Exception e) {
            log.warn("动画立绘生成失败 cost={}ms", System.currentTimeMillis() - t0, e);
            return err("生成失败: " + e.getMessage());
        }
    }

    private byte[] removeBlackBg(byte[] webpData) throws Exception {
        java.nio.file.Path tmp = java.nio.file.Files.createTempFile("mmo-anim-", ".webp");
        java.nio.file.Path out = java.nio.file.Files.createTempFile("mmo-anim-alpha-", ".webp");
        try {
            java.nio.file.Files.write(tmp, webpData);
            ProcessBuilder pb = new ProcessBuilder(
                    "C:\\ComfyUI_windows_portable\\python_embeded\\python.exe",
                    "C:\\ComfyUI_windows_portable\\remove_bg.py",
                    tmp.toString(), out.toString());
            pb.redirectErrorStream(true);
            Process proc = pb.start();
            String output = new String(proc.getInputStream().readAllBytes());
            int code = proc.waitFor();
            if (code != 0) throw new java.io.IOException("remove_bg failed: " + output);
            return java.nio.file.Files.readAllBytes(out);
        } finally {
    ...
    public ResponseEntity<Map<String, Object>> getPersonInfo(HttpSession session) {
        ...
        // 动画立绘URL
        var apList = animatedPortraitRepository.findByGroupKeyOrderByCreateTimeDesc("anim_person_" + userId);
        if (!apList.isEmpty()) {
            m.put("animatedPortraitUrl", "/api/animated-portrait/" + apList.get(0).getId());
            m.put("animatedPortraits", apList.stream().map(ap -> Map.of(
                    "id", ap.getId(), "url", "/api/animated-portrait/" + ap.getId(),
                    "prompt", ap.getPrompt() != null ? ap.getPrompt() : "", "favorite", ap.isFavorite(),
                    "createTime", ap.getCreateTime())).toList());
        }
        return ok(m);
    }
    ...
    public ResponseEntity<Map<String, Object>> generateSubjectPortrait(@RequestBody Map<String, Object> body, HttpSession session) throws Exception {
        ...
        String targetId = (String) body.get("targetId");
        String style = (String) body.getOrDefault("style", "仙侠水墨风");
        int width = ((Number) body.getOrDefault("width", 768)).intValue();
        int height = ((Number) body.getOrDefault("height", 1024)).intValue();
        ...
        switch (target) {
            case "companion" -> {
                ...
                var result = runImageTask(() -> sceneImageService.getOrGenerate(cacheKey, buildCompanionPortraitPrompt(comp, style), width, height));
            ...
            case "pet" -> {
                ...
                var result = runImageTask(() -> sceneImageService.getOrGenerate(cacheKey, buildPetPortraitPrompt(pet, style), width, height));
            ...
            default -> {
                ...
                var result = runImageTask(() -> sceneImageService.getOrGenerate(cacheKey, buildPortraitPrompt(person.getName(), style, person.getGender(), person.getFeatures()), width, height));
    ...
    private String buildCompanionPortraitPrompt(SpiritCompanion comp, String style) {
        return style + "灵侣立绘，" + comp.getName() + "，"
                ...
                + "仙侠角色，气质出众，"
                + "单人全身立绘（从头顶到脚底完整呈现），正面或四分之三侧面，"
                + "全身可见包含双腿和双脚，占画面85%以上，面部精致，表情生动，"
                + "姿态飘逸，主体居中，"
    ...
    private String buildPortraitPrompt(String name, String style, String gender, String features) {
        ...
        sb.append("，单人全身立绘（从头顶到脚底完整呈现），正面或四分之三侧面，");
        sb.append("人物占画面85%以上，面部清晰精致，表情生动，");
        sb.append("全身可见包含双腿和双脚，姿态飘逸，主体居中，");
        sb.append("【颜色要求】头发和服饰禁止使用纯黑色，服饰使用明亮鲜艳色系，");
    ...
    public ResponseEntity<Map<String, Object>> unlockSkill(@RequestBody Map<String, String> body, HttpSession session) {
        long userId = requireLogin(session);
        SkillService.UnlockResult result = skillService.unlockSkillAndCharge(userId, body.get("skillTemplateId"));
        PlayerCurrency currency = shopService.getPlayerCurrency(userId);
        PlayerSkill unlocked = result.skill();
        return ok(Map.of("skillTemplateId", unlocked.getSkillTemplateId(),
                "level", unlocked.getLevel(), "unlocked", unlocked.isUnlocked(),
    ...
    /** 为战斗单位附加立绘URL：玩家用已有立绘，怪物AI生成；优先查找动画版 */
    private void attachBattlePortraits(long userId, BattleState state) {
        ...
        // 玩家动画立绘
        animatedPortraitRepository.findFirstByGroupKeyOrderByCreateTimeDesc("anim_person_" + userId)
                .ifPresent(ap -> state.getPlayerUnits()
                        .forEach(u -> u.setAnimatedPortraitUrl("/api/animated-portrait/" + ap.getId())));
        ...
        for (BattleUnit enemy : state.getEnemyUnits()) {
            ...
            // 怪物动画立绘
            animatedPortraitRepository.findFirstByGroupKeyOrderByCreateTimeDesc("anim_monster_" + enemy.getName())
                    .ifPresent(ap -> enemy.setAnimatedPortraitUrl("/api/animated-portrait/" + ap.getId()));
        }
    ...
    private Map<String, Object> unitToMap(BattleUnit u) {
        ...
        m.put("animatedPortraitUrl", u.getAnimatedPortraitUrl());
        return m;
    }
    ...
    public ResponseEntity<Map<String, Object>> marketSell(@RequestBody Map<String, Object> body, HttpSession session) {
        ...
        try {
            ...
            // Do not destroy the item if persistence rejects the listing.
            BagItem restore = new BagItem();
            restore.setId(itemId);
            restore.setItemTypeId(itemId);
            restore.setQuantity(quantity);
            bagService.incrementItem(restore, userId);
            return err("上架失败，物品已退回");
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> marketBuy(@RequestBody Map<String, Object> body, HttpSession session) {
        ...
        int observedSold = listing.getSold();
        com.iohao.mmo.trade.entity.MarketListing reserved;
        try {
            // Reserve inside TradeService first so concurrent buyers are charged
            // only for the quantity that was actually available.
            reserved = tradeService.executeBuy(userId, listingId, quantity);
        } catch (IllegalArgumentException e) {
            return err(e.getMessage());
        }
        int bought = reserved.getSold() - observedSold;
        if (bought <= 0) return err("库存不足");

        long totalCost;
        try {
            totalCost = Math.multiplyExact(reserved.getUnitPrice(), bought);
        } catch (ArithmeticException e) {
            tradeService.rollbackBuy(listingId, bought);
            return err("交易金额超出上限");
        }
        ...
        if (!shopService.trySpendGold(userId, (int) totalCost)) {
            tradeService.rollbackBuy(listingId, bought);
            return err("金币不足");
        }
        ...
        try {
            boolean stackable = isStackableCategory(reserved.getItemCategory());
            if (stackable) {
                BagItem toAdd = new BagItem();
                toAdd.setId(reserved.getItemId());
                toAdd.setItemTypeId(reserved.getItemId());
            ...
            tradeService.rollbackBuy(listingId, bought);
            shopService.refundGold(userId, (int) totalCost);
            log.warn("集市购买交付失败，已回滚 buyer={} listing={} qty={}", userId, listingId, bought, deliveryFailure);
            return err("物品交付失败，金币已退回");
        }

        // 物品交付成功后再给卖家结算，避免买家收到物品但卖家结算失败。
        long fee = Math.max(1L, totalCost * 5 / 100);
        shopService.addCurrency(reserved.getSellerId(), (int) (totalCost - fee), 0);
    ...
    private final ConcurrentHashMap<Long, Map<String, Object>> arenaState = new ConcurrentHashMap<>();

    @GetMapping("/arena/match")
    public ResponseEntity<Map<String, Object>> arenaMatch(HttpSession session) {
        long userId = requireLogin(session);
        List<Map<String, Object>> opponents = List.of(
            Map.of("id", 1001L, "name", "剑圣·无名", "level", 45, "power", 12800),
            Map.of("id", 1002L, "name", "暗影刺客", "level", 43, "power", 11500),
            Map.of("id", 1003L, "name", "火焰法师", "level", 47, "power", 13200)
        );
        Map<String, Object> myState = arenaState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "rank", 999, "wins", 0, "losses", 0, "tickets", 5
        )));
        return ok(Map.of("opponents", opponents, "state", myState));
    }

    @PostMapping("/arena/challenge")
    public ResponseEntity<Map<String, Object>> arenaChallenge(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        long opponentId = ((Number) body.getOrDefault("opponentId", 0)).longValue();
        Map<String, Object> state = arenaState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "rank", 999, "wins", 0, "losses", 0, "tickets", 5
        )));
        boolean win = new Random().nextBoolean();
        state.put("wins", ((Number) state.get("wins")).intValue() + (win ? 1 : 0));
        state.put("losses", ((Number) state.get("losses")).intValue() + (win ? 0 : 1));
        if (win) state.put("rank", Math.max(1, ((Number) state.get("rank")).intValue() - 1));
        return ok(Map.of("win", win, "opponentId", opponentId, "state", state));
    }

    // ── Fishing 钓鱼 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> fishingState = new ConcurrentHashMap<>();

    @GetMapping("/fishing/status")
    public ResponseEntity<Map<String, Object>> fishingStatus(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = fishingState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "casting", false, "bait", "蚯蚓", "rodLevel", 1, "catches", new CopyOnWriteArrayList<>()
        )));
        return ok(state);
    }

    @PostMapping("/fishing/cast")
    public ResponseEntity<Map<String, Object>> fishingCast(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = fishingState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "casting", false, "bait", "蚯蚓", "rodLevel", 1, "catches", new CopyOnWriteArrayList<>()
        )));
        state.put("casting", true);
        state.put("castTime", System.currentTimeMillis());
        return ok(Map.of("casting", true, "message", "鱼线已抛出，等待鱼儿上钩..."));
    }

    @PostMapping("/fishing/reel")
    public ResponseEntity<Map<String, Object>> fishingReel(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = fishingState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "casting", false, "bait", "蚯蚓", "rodLevel", 1, "catches", new CopyOnWriteArrayList<>()
        )));
        state.put("casting", false);
        String[] fishTypes = {"鲫鱼", "鲤鱼", "金龙鱼", "灵鱼", "废弃靴子"};
        String caught = fishTypes[new Random().nextInt(fishTypes.length)];
        @SuppressWarnings("unchecked")
        List<Object> catches = (List<Object>) state.get("catches");
        catches.add(Map.of("name", caught, "time", System.currentTimeMillis()));
        return ok(Map.of("caught", caught, "totalCatches", catches.size()));
    }

    // ── Housing 家园 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> housingState = new ConcurrentHashMap<>();

    @GetMapping("/housing/state")
    public ResponseEntity<Map<String, Object>> housingState(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = housingState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "level", 1, "rooms", List.of("客厅", "卧室"), "furniture", 3, "comfort", 20
        )));
        return ok(state);
    }

    @PostMapping("/housing/upgrade")
    public ResponseEntity<Map<String, Object>> housingUpgrade(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        Map<String, Object> state = housingState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "level", 1, "rooms", List.of("客厅", "卧室"), "furniture", 3, "comfort", 20
        )));
        int level = ((Number) state.get("level")).intValue() + 1;
        state.put("level", level);
        state.put("comfort", ((Number) state.get("comfort")).intValue() + 15);
        return ok(Map.of("level", level, "message", "家园升级成功！当前等级: " + level));
    }

    // ── Mount 坐骑 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> mountState = new ConcurrentHashMap<>();

    @GetMapping("/mount/list")
    public ResponseEntity<Map<String, Object>> mountList(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = mountState.computeIfAbsent(userId, k -> {
            ConcurrentHashMap<String, Object> m = new ConcurrentHashMap<>();
            m.put("equipped", "");
            m.put("mounts", List.of(
                Map.of("id", "m1", "name", "白马", "speed", 120, "rarity", "普通"),
                Map.of("id", "m2", "name", "火麒麟", "speed", 180, "rarity", "稀有"),
                Map.of("id", "m3", "name", "暗影龙", "speed", 250, "rarity", "传说")
            ));
            return m;
        });
        return ok(state);
    ...
    @PostMapping("/mount/equip")
    public ResponseEntity<Map<String, Object>> mountEquip(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        String mountId = (String) body.getOrDefault("mountId", "m1");
        Map<String, Object> state = mountState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of("equipped", "", "mounts", List.of())));
        state.put("equipped", mountId);
        return ok(Map.of("equipped", mountId, "message", "坐骑已装备"));
    }

    // ── Fashion 时装 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> fashionState = new ConcurrentHashMap<>();

    @GetMapping("/fashion/list")
    public ResponseEntity<Map<String, Object>> fashionList(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = fashionState.computeIfAbsent(userId, k -> {
            ConcurrentHashMap<String, Object> m = new ConcurrentHashMap<>();
            m.put("equipped", "");
            m.put("items", List.of(
                Map.of("id", "f1", "name", "星辰战甲", "type", "armor", "bonus", "+5% 防御"),
                Map.of("id", "f2", "name", "月光长裙", "type", "dress", "bonus", "+3% 魅力"),
                Map.of("id", "f3", "name", "烈焰披风", "type", "cape", "bonus", "+5% 攻击")
            ));
            return m;
        });
        return ok(state);
    ...
    @PostMapping("/fashion/equip")
    public ResponseEntity<Map<String, Object>> fashionEquip(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        String itemId = (String) body.getOrDefault("itemId", "f1");
        Map<String, Object> state = fashionState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of("equipped", "", "items", List.of())));
        state.put("equipped", itemId);
        return ok(Map.of("equipped", itemId, "message", "时装已穿戴"));
    }

    // ── Notification 消息通知 ──────────────────────────────────

    private final ConcurrentHashMap<Long, CopyOnWriteArrayList<Map<String, Object>>> notifyFeed = new ConcurrentHashMap<>();

    @GetMapping("/notify/feed")
    public ResponseEntity<Map<String, Object>> notifyFeed(HttpSession session) {
        long userId = requireLogin(session);
        CopyOnWriteArrayList<Map<String, Object>> feed = notifyFeed.computeIfAbsent(userId, k -> {
            CopyOnWriteArrayList<Map<String, Object>> list = new CopyOnWriteArrayList<>();
            list.add(Map.of("id", "n1", "type", "system", "title", "欢迎回来", "content", "每日奖励已发放", "read", false, "time", System.currentTimeMillis()));
            list.add(Map.of("id", "n2", "type", "battle", "title", "竞技场挑战", "content", "有人向你发起了挑战", "read", false, "time", System.currentTimeMillis()));
            list.add(Map.of("id", "n3", "type", "trade", "title", "交易完成", "content", "拍卖物品已售出", "read", true, "time", System.currentTimeMillis()));
            return list;
        });
        long unread = feed.stream().filter(n -> Boolean.FALSE.equals(n.get("read"))).count();
        return ok(Map.of("notifications", feed, "unread", unread));
    }

    @PostMapping("/notify/read")
    public ResponseEntity<Map<String, Object>> notifyRead(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        String notifId = (String) body.getOrDefault("id", "");
        CopyOnWriteArrayList<Map<String, Object>> feed = notifyFeed.computeIfAbsent(userId, k -> new CopyOnWriteArrayList<>());
        for (Map<String, Object> n : feed) {
            if (notifId.equals(n.get("id"))) {
                Map<String, Object> updated = new HashMap<>(n);
                updated.put("read", true);
                feed.set(feed.indexOf(n), updated);
                break;
            }
        }
        return ok(Map.of("success", true));
    ...
    private final ConcurrentHashMap<Long, Map<String, Object>> bankState = new ConcurrentHashMap<>();

    @GetMapping("/bank/balance")
    public ResponseEntity<Map<String, Object>> bankBalance(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = bankState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "gold", 10000L, "deposit", 0L, "interest", 0.05
        )));
        return ok(state);
    }

    @PostMapping("/bank/deposit")
    public ResponseEntity<Map<String, Object>> bankDeposit(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        long amount = ((Number) body.getOrDefault("amount", 0)).longValue();
        Map<String, Object> state = bankState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "gold", 10000L, "deposit", 0L, "interest", 0.05
        )));
        long gold = ((Number) state.get("gold")).longValue();
        if (amount <= 0 || amount > gold) return ok(Map.of("success", false, "message", "金额无效"));
        state.put("gold", gold - amount);
        state.put("deposit", ((Number) state.get("deposit")).longValue() + amount);
        return ok(Map.of("success", true, "gold", state.get("gold"), "deposit", state.get("deposit")));
    }

    @PostMapping("/bank/withdraw")
    public ResponseEntity<Map<String, Object>> bankWithdraw(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        long amount = ((Number) body.getOrDefault("amount", 0)).longValue();
        Map<String, Object> state = bankState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "gold", 10000L, "deposit", 0L, "interest", 0.05
        )));
        long deposit = ((Number) state.get("deposit")).longValue();
        if (amount <= 0 || amount > deposit) return ok(Map.of("success", false, "message", "金额无效"));
        state.put("deposit", deposit - amount);
        state.put("gold", ((Number) state.get("gold")).longValue() + amount);
        return ok(Map.of("success", true, "gold", state.get("gold"), "deposit", state.get("deposit")));
    }

    // ── Battlefield 战场 ──────────────────────────────────

    @GetMapping("/battlefield/info")
    public ResponseEntity<Map<String, Object>> battlefieldInfo(HttpSession session) {
        requireLogin(session);
        return ok(Map.of(
            "name", "烈焰战场",
            "status", "open",
            ...
            "maxPlayers", 40,
            "timeRemaining", 1200,
            "rewards", List.of("荣誉点 x100", "战场宝箱 x1", "经验 x5000")
        ));
    }

    @PostMapping("/battlefield/join")
    public ResponseEntity<Map<String, Object>> battlefieldJoin(HttpSession session) {
        long userId = requireLogin(session);
        return ok(Map.of("joined", true, "team", "红方", "position", new Random().nextInt(20) + 1, "message", "已加入战场，准备战斗！"));
    }

    // ── Ghost House 鬼屋 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> ghostHouseState = new ConcurrentHashMap<>();

    @PostMapping("/ghosthouse/enter")
    public ResponseEntity<Map<String, Object>> ghostHouseEnter(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = new ConcurrentHashMap<>(Map.of(
            "floor", 1, "hp", 100, "courage", 50, "events", new CopyOnWriteArrayList<>()
        ));
        ghostHouseState.put(userId, state);
        return ok(Map.of("entered", true, "floor", 1, "message", "你踏入了幽暗的鬼屋..."));
    }

    @PostMapping("/ghosthouse/explore")
    public ResponseEntity<Map<String, Object>> ghostHouseExplore(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = ghostHouseState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "floor", 1, "hp", 100, "courage", 50, "events", new CopyOnWriteArrayList<>()
        )));
        String[] events = {"发现宝箱，获得灵石 x10", "遭遇恶灵，失去 10 HP", "找到密道，前进一层", "触发机关，勇气 -10", "遇到神秘商人"};
        String event = events[new Random().nextInt(events.length)];
        int floor = ((Number) state.get("floor")).intValue();
        if (event.contains("前进")) { floor++; state.put("floor", floor); }
        if (event.contains("HP")) state.put("hp", ((Number) state.get("hp")).intValue() - 10);
        if (event.contains("勇气")) state.put("courage", ((Number) state.get("courage")).intValue() - 10);
        return ok(Map.of("event", event, "floor", floor, "hp", state.get("hp"), "courage", state.get("courage")));
    }

    // ── Shooting 射击 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> shootingState = new ConcurrentHashMap<>();

    @PostMapping("/shooting/start")
    public ResponseEntity<Map<String, Object>> shootingStart(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = new ConcurrentHashMap<>(Map.of(
            "score", 0, "ammo", 30, "targets", 10, "hits", 0, "active", true
        ));
        shootingState.put(userId, state);
        return ok(Map.of("started", true, "ammo", 30, "targets", 10));
    }

    @PostMapping("/shooting/score")
    public ResponseEntity<Map<String, Object>> shootingScore(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        Map<String, Object> state = shootingState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "score", 0, "ammo", 30, "targets", 10, "hits", 0, "active", true
        )));
        boolean hit = new Random().nextInt(100) < 70;
        int ammo = ((Number) state.get("ammo")).intValue() - 1;
        state.put("ammo", ammo);
        if (hit) {
            state.put("hits", ((Number) state.get("hits")).intValue() + 1);
            state.put("score", ((Number) state.get("score")).intValue() + 100);
        }
        if (ammo <= 0) state.put("active", false);
        return ok(Map.of("hit", hit, "score", state.get("score"), "ammo", ammo, "active", state.get("active")));
    }

    // ── Lineage 血脉 ──────────────────────────────────

    @GetMapping("/lineage/tree")
    public ResponseEntity<Map<String, Object>> lineageTree(HttpSession session) {
        long userId = requireLogin(session);
        List<Map<String, Object>> ancestors = List.of(
            Map.of("generation", 1, "name", "始祖·玄天", "bloodline", "龙脉", "bonus", "+10% 全属性"),
            Map.of("generation", 2, "name", "二代·青云", "bloodline", "龙脉", "bonus", "+5% 攻击"),
            Map.of("generation", 3, "name", "三代·明月", "bloodline", "龙脉", "bonus", "+3% 防御")
        );
        return ok(Map.of("userId", userId, "bloodline", "龙脉", "purity", 72, "ancestors", ancestors));
    }

    // ── Master-Disciple 师徒 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> masterState = new ConcurrentHashMap<>();

    @GetMapping("/master/info")
    public ResponseEntity<Map<String, Object>> masterInfo(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = masterState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "role", "none", "masterId", 0L, "disciples", List.of(), "contribution", 0
        )));
        return ok(state);
    }

    @PostMapping("/master/apply")
    public ResponseEntity<Map<String, Object>> masterApply(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        String role = (String) body.getOrDefault("role", "disciple");
        Map<String, Object> state = masterState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>());
        state.put("role", role);
        if ("master".equals(role)) {
            state.put("disciples", List.of());
            return ok(Map.of("success", true, "message", "已注册为师父，等待徒弟拜师"));
        } else {
            state.put("masterId", 1001L);
            return ok(Map.of("success", true, "message", "拜师成功！师父：剑圣·无名"));
        }
    }

    // ── Mirage 幻境 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> mirageState = new ConcurrentHashMap<>();

    @GetMapping("/mirage/state")
    public ResponseEntity<Map<String, Object>> mirageState(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = mirageState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "layer", 0, "active", false, "illusions", 0, "rewards", List.of()
        )));
        return ok(state);
    }

    @PostMapping("/mirage/enter")
    public ResponseEntity<Map<String, Object>> mirageEnter(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = mirageState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>());
        int layer = ((Number) state.getOrDefault("layer", 0)).intValue() + 1;
        state.put("layer", layer);
        state.put("active", true);
        String[] illusions = {"镜花水月", "虚空迷阵", "心魔试炼", "天地倒转"};
        String illusion = illusions[new Random().nextInt(illusions.length)];
        state.put("illusions", ((Number) state.getOrDefault("illusions", 0)).intValue() + 1);
        return ok(Map.of("layer", layer, "illusion", illusion, "message", "进入第 " + layer + " 层幻境: " + illusion));
    }

    // ── Monthly Card 月卡 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> monthlyCardState = new ConcurrentHashMap<>();

    @GetMapping("/monthly-card/state")
    public ResponseEntity<Map<String, Object>> monthlyCardState(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = monthlyCardState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "active", false, "daysRemaining", 0, "todayClaimed", false, "totalClaimed", 0
        )));
        return ok(state);
    }

    @PostMapping("/monthly-card/claim")
    public ResponseEntity<Map<String, Object>> monthlyCardClaim(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = monthlyCardState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "active", true, "daysRemaining", 30, "todayClaimed", false, "totalClaimed", 0
        )));
        if (Boolean.TRUE.equals(state.get("todayClaimed"))) {
            return ok(Map.of("success", false, "message", "今日已领取"));
        }
        state.put("todayClaimed", true);
        state.put("totalClaimed", ((Number) state.getOrDefault("totalClaimed", 0)).intValue() + 1);
        return ok(Map.of("success", true, "reward", "玩币 x100 + 经验 x2000", "totalClaimed", state.get("totalClaimed")));
    }

    // ── First Recharge 首充 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> firstRechargeState = new ConcurrentHashMap<>();

    @GetMapping("/first-recharge/state")
    public ResponseEntity<Map<String, Object>> firstRechargeState(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = firstRechargeState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "recharged", false, "claimed", false,
            "rewards", List.of("金装武器 x1", "坐骑·火麒麟", "玩币 x5000", "经验丹 x10")
        )));
        return ok(state);
    }

    @PostMapping("/first-recharge/claim")
    public ResponseEntity<Map<String, Object>> firstRechargeClaim(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = firstRechargeState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "recharged", true, "claimed", false,
            "rewards", List.of("金装武器 x1", "坐骑·火麒麟", "玩币 x5000", "经验丹 x10")
        )));
        if (Boolean.TRUE.equals(state.get("claimed"))) {
            return ok(Map.of("success", false, "message", "奖励已领取"));
        }
        state.put("claimed", true);
        return ok(Map.of("success", true, "message", "首充奖励领取成功！", "rewards", state.get("rewards")));
    }

    // ── Rune Furnace 符文熔炉 ──────────────────────────────────

    @GetMapping("/rune-furnace/recipes")
    public ResponseEntity<Map<String, Object>> runeFurnaceRecipes(HttpSession session) {
        requireLogin(session);
        List<Map<String, Object>> recipes = List.of(
            Map.of("id", "r1", "name", "烈焰符文", "materials", List.of("火元素 x5", "符文石 x2"), "successRate", 80),
            Map.of("id", "r2", "name", "寒冰符文", "materials", List.of("水元素 x5", "符文石 x2"), "successRate", 80),
            Map.of("id", "r3", "name", "雷霆符文", "materials", List.of("雷元素 x5", "符文石 x3"), "successRate", 60),
            Map.of("id", "r4", "name", "圣光符文", "materials", List.of("光元素 x8", "符文石 x5"), "successRate", 40)
        );
        return ok(Map.of("recipes", recipes));
    }

    @PostMapping("/rune-furnace/refine")
    public ResponseEntity<Map<String, Object>> runeFurnaceRefine(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        String recipeId = (String) body.getOrDefault("recipeId", "r1");
        int rate = recipeId.equals("r4") ? 40 : recipeId.equals("r3") ? 60 : 80;
        boolean success = new Random().nextInt(100) < rate;
        return ok(Map.of("success", success, "recipeId", recipeId,
            "message", success ? "炼制成功！获得符文" : "炼制失败，材料消耗"));
    }

    // ── Soul Attach 附魂 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> soulState = new ConcurrentHashMap<>();

    @GetMapping("/soul/state")
    public ResponseEntity<Map<String, Object>> soulState(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = soulState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "souls", List.of(
                Map.of("id", "s1", "name", "战魂", "level", 1, "attached", false),
                Map.of("id", "s2", "name", "守魂", "level", 1, "attached", false),
                Map.of("id", "s3", "name", "灵魂", "level", 2, "attached", true)
            ),
            "slots", 3, "activeSouls", 1
        )));
        return ok(state);
    }

    @PostMapping("/soul/attach")
    public ResponseEntity<Map<String, Object>> soulAttach(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        String soulId = (String) body.getOrDefault("soulId", "s1");
        return ok(Map.of("success", true, "soulId", soulId, "message", "附魂成功，属性已提升"));
    }

    // ── Soul Tower 镇妖塔 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> towerState = new ConcurrentHashMap<>();

    @GetMapping("/tower/state")
    public ResponseEntity<Map<String, Object>> towerState(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = towerState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "currentFloor", 1, "maxFloor", 100, "bestFloor", 1, "dailyAttempts", 3
        )));
        return ok(state);
    }

    @PostMapping("/tower/challenge")
    public ResponseEntity<Map<String, Object>> towerChallenge(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = towerState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "currentFloor", 1, "maxFloor", 100, "bestFloor", 1, "dailyAttempts", 3
        )));
        int floor = ((Number) state.get("currentFloor")).intValue();
        boolean cleared = new Random().nextInt(100) < 75;
        if (cleared) {
            floor++;
            state.put("currentFloor", floor);
            int best = ((Number) state.get("bestFloor")).intValue();
            if (floor > best) state.put("bestFloor", floor);
        }
        state.put("dailyAttempts", ((Number) state.get("dailyAttempts")).intValue() - 1);
        return ok(Map.of("cleared", cleared, "floor", floor, "reward", cleared ? "灵石 x" + (floor * 10) : "无",
            "dailyAttempts", state.get("dailyAttempts")));
    }

    // ── Tutorial 新手引导 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> tutorialState = new ConcurrentHashMap<>();

    @GetMapping("/tutorial/steps")
    public ResponseEntity<Map<String, Object>> tutorialSteps(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = tutorialState.computeIfAbsent(userId, k -> {
            ConcurrentHashMap<String, Object> m = new ConcurrentHashMap<>();
            m.put("completed", false);
            m.put("currentStep", 1);
            m.put("steps", List.of(
                Map.of("step", 1, "title", "创建角色", "done", true),
                Map.of("step", 2, "title", "首次战斗", "done", false),
                Map.of("step", 3, "title", "装备强化", "done", false),
                Map.of("step", 4, "title", "加入帮派", "done", false),
                Map.of("step", 5, "title", "完成首次副本", "done", false)
            ));
            return m;
        });
        return ok(state);
    ...
    public ResponseEntity<Map<String, Object>> tutorialComplete(HttpSession session, @RequestBody Map<String, Object> body) {
        long userId = requireLogin(session);
        int step = ((Number) body.getOrDefault("step", 1)).intValue();
        Map<String, Object> state = tutorialState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>());
        state.put("currentStep", step + 1);
        if (step >= 5) state.put("completed", true);
        return ok(Map.of("step", step, "completed", step >= 5, "message", "步骤 " + step + " 完成！"));
    }

    // ── Couple 双修 ──────────────────────────────────

    private final ConcurrentHashMap<Long, Map<String, Object>> coupleState = new ConcurrentHashMap<>();

    @GetMapping("/couple/state")
    public ResponseEntity<Map<String, Object>> coupleState(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = coupleState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "partnerId", 0L, "partnerName", "", "level", 0, "exp", 0L, "dailyCultivate", 3, "bonuses", List.of("+5% 双攻", "+3% 双防")
        )));
        return ok(state);
    }

    @PostMapping("/couple/cultivate")
    public ResponseEntity<Map<String, Object>> coupleCultivate(HttpSession session) {
        long userId = requireLogin(session);
        Map<String, Object> state = coupleState.computeIfAbsent(userId, k -> new ConcurrentHashMap<>(Map.of(
            "partnerId", 0L, "partnerName", "", "level", 0, "exp", 0L, "dailyCultivate", 3, "bonuses", List.of()
        )));
        int remaining = ((Number) state.getOrDefault("dailyCultivate", 0)).intValue();
        if (remaining <= 0) return ok(Map.of("success", false, "message", "今日修炼次数已用完"));
        long exp = ((Number) state.getOrDefault("exp", 0L)).longValue() + 500;
        state.put("exp", exp);
        state.put("dailyCultivate", remaining - 1);
        int level = (int) (exp / 2000);
        state.put("level", level);
        return ok(Map.of("success", true, "exp", exp, "level", level, "remaining", remaining - 1, "message", "双修完成，获得 500 经验"));
    }
