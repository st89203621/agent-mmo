...
public class TestGenerate {
    public static void main(String[] args) throws Exception {
        boolean unity = args.length > 0 && "--unity".equals(args[0]);
        Path output = Path.of(args.length > 1 ? args[1] : "target/unity-protocol").toAbsolutePath();
        extractedTrick();
        ...
        if (unity) {
            documentGenerate.setPath(output.resolve("routes").toString());
            documentGenerate.setNamespace("Lunhui.Protocol.Routes");
            documentGenerate.setProtoImportPath("using Lunhui.Protocol;");
        }
        ...
        // .proto 文件生成
        if (unity) {
            UnityProtocolGenerate.main(new String[]{output.toString()});
        } else {
            generateProtoFile();
        }
    }
