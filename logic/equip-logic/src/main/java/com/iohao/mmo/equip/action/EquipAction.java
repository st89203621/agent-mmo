...
public class EquipAction {
    ...
    public List<EquipMessage> getEquipList(FlowContext flowContext) {
        ...
        EquipLoadout loadout = equipService.loadout(userId);
        List<Equip> equipList = equipService.listByUser(userId);
        List<EquipMessage> result = EquipMapper.ME.convert(equipList);
        result.forEach(equip -> equip.equipped = java.util.Objects.equals(equip.id, loadout.getSlots().get(equip.position)));
        return result;
    }
    ...
    public EquipMessage getEquip(String equipId, FlowContext flowContext) {
        Equip equip = equipService.findById(equipId);
        GameCode.objNotFound.assertTrue(equip != null && equip.getUserId() == flowContext.getUserId());
    ...
    public void allotEquip(EquipMessage equipMessage, FlowContext flowContext) {
        Equip old = equipService.findById(equipMessage.id);
        GameCode.objNotFound.assertTrue(old != null && old.getUserId() == flowContext.getUserId());
    ...
    public EquipMessage resetEquip(EquipResetMessage equipResetMessage, FlowContext flowContext) {
        ...
        Equip old = equipService.findById(equipResetMessage.id);
        GameCode.objNotFound.assertTrue(old != null && old.getUserId() == flowContext.getUserId());
    ...
    public void delEquipBatch(List<String> idList, FlowContext flowContext) {
        GameCode.objNotFound.assertTrue(idList != null && idList.size() <= 100);
    ...
    @ActionMethod(EquipCmd.upgradeGrade)
    public EquipMessage upgradeGrade(String equipId, FlowContext flowContext) {
        return EquipMapper.ME.convert(equipService.upgradeGrade(equipId, flowContext.getUserId()));
    }

    @ActionMethod(EquipCmd.furnaceUpgrade)
    public EquipMessage furnaceUpgrade(String equipId, FlowContext flowContext) {
        return EquipMapper.ME.convert(equipService.furnaceUpgrade(equipId, flowContext.getUserId()));
    }

    @ActionMethod(EquipCmd.wearEquip)
    public List<EquipMessage> wearEquip(String equipId, FlowContext flowContext) {
        equipService.wearEquip(flowContext.getUserId(), equipId);
        return getEquipList(flowContext);
    }
}
