...
public class EquipService {
    ...
    public void allotEquip(Equip equip){
        ...
        GameCode.allotNotEnough.assertNonNull(elseEquipProperty);
        GameCode.allotNotEnough.assertTrue(elseEquipProperty.getConstitution() >= 0
                && elseEquipProperty.getMagicPower() >= 0 && elseEquipProperty.getPower() >= 0
                && elseEquipProperty.getEndurance() >= 0 && elseEquipProperty.getAgile() >= 0);
        ...
        GameCode.allotNotEnough.assertTrue(saveAttrTotal >= 0 && saveAttrTotal<=oldEquip.getAttrTotal());
    ...
    public synchronized EquipLoadout loadout(long userId) {
        EquipLoadout loadout = mongoTemplate.findById(userId, EquipLoadout.class);
        if (loadout == null) { loadout = new EquipLoadout(); loadout.setUserId(userId); }
        if (!loadout.isStarterGranted()) {
            String id = "starter_weapon_" + userId;
            if (findById(id) == null) {
                save(Equip.builder().id(id).userId(userId).itemTypeId("weapon_006").position(2)
                        .level(1).grade(1).quality(1).attrTotal(20).undistributedAttr(20)
                        .totalAttrMin(20).totalAttrMax(40)
                        .fixedEquipProperty(FixedEquipProperty.builder().physicsAttack(30).build())
                        .elseEquipProperty(ElseEquipProperty.resetElseEquipProperty()).build());
            }
            loadout.getSlots().putIfAbsent(2, id);
            loadout.setStarterGranted(true);
            mongoTemplate.save(loadout);
        }
        return loadout;
    }

    public synchronized EquipLoadout wearEquip(long userId, String equipId) {
        Equip equip = findById(equipId);
        GameCode.objNotFound.assertTrue(equip != null && equip.getUserId() == userId && equip.getLevel() <= playerLevel(userId));
        EquipLoadout loadout = loadout(userId);
        loadout.getSlots().put(equip.getPosition(), equipId);
        return mongoTemplate.save(loadout);
    }

    public synchronized Equip upgradeGrade(String id, long userId) {
        Equip equip = findById(id);
        GameCode.objNotFound.assertTrue(equip != null && equip.getUserId() == userId);
        GameCode.objNotFound.assertTrue(playerLevel(userId) >= 3 && equip.getGrade() < MAX_GRADE);
        int target = equip.getGrade() + 1;
        shopService.consumeCraftCost(userId, target * 1800, "material_002", target * 5);
        return upgradeGrade(id);
    }
    ...
    public synchronized Equip furnaceUpgrade(String id, long userId) {
        Equip equip = findById(id);
        GameCode.objNotFound.assertTrue(equip != null && equip.getUserId() == userId);
        GameCode.objNotFound.assertTrue(equip.getGrade() >= MAX_GRADE && equip.getFurnaceGrade() < MAX_FURNACE);
        shopService.consumeCraftCost(userId, (equip.getFurnaceGrade() + 1) * 2500, "material_004", 1);
        return furnaceUpgrade(id);
    }

    private int playerLevel(long userId) {
        Document level = mongoTemplate.findById(userId, Document.class, "level");
        return level == null ? 1 : level.getInteger("level", 1);
    }
