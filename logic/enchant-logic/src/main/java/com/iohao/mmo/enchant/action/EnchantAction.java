...
public class EnchantAction {
    ...
    public EnchantMessage enchantEquip(EnchantMessage message, FlowContext flowContext) {
        ...
        int before = enchantService.getOrCreateEnchant(message.equipId, userId).getEnchantLevel();
        ...
        EnchantMessage result = EnchantMapper.ME.convert(enchant);
        result.success = enchant.getEnchantLevel() > before;
        return result;
    }
    ...
    @ActionMethod(EnchantCmd.prestigeEnchant)
    public EnchantMessage prestigeEnchant(EnchantMessage message, FlowContext flowContext) {
        int before = enchantService.getOrCreateEnchant(message.equipId, flowContext.getUserId()).getEnchantLevel();
        EquipEnchant enchant = enchantService.prestigeEnchant(message.equipId, flowContext.getUserId());
        EnchantMessage result = EnchantMapper.ME.convert(enchant);
        result.success = enchant.getEnchantLevel() > before;
        return result;
    }
