...
public class EnchantService {
    ...
    public EquipEnchant getOrCreateEnchant(String equipId, long userId) {
        GameCode.objNotFound.assertTrue(equipId != null && !equipId.isBlank());
        ...
        GameCode.objNotFound.assertTrue(enchant.getUserId() == userId);
        return enchant;
    }
     ...
     * 使用符文，按等级有不同成功率和增长范围
     */
    public synchronized EquipEnchant enchantEquip(String equipId, long userId, EnchantRune.RuneLevel runeLevel) {
        ...
        requireLevel(userId);
        int target = enchant.getEnchantLevel() + 1;
        shopService.consumeCraftCost(userId, target * 1500, "material_001", target * 3);
     ...
     * 失败降2级
     */
    public synchronized EquipEnchant prestigeEnchant(String equipId, long userId) {
        EquipEnchant enchant = getOrCreateEnchant(equipId, userId);
        ...
        requireLevel(userId);
        shopService.consumeCraftCost(userId, (enchant.getEnchantLevel() + 1) * 2000, "material_003", 3);
    ...
    private void requireLevel(long userId) {
        Document level = mongoTemplate.findById(userId, Document.class, "level");
        GameCode.objNotFound.assertTrue(level != null && level.getInteger("level", 1) >= 8);
    }
}
