...
public class TitleService {
    ...
    public boolean equipTitle(long playerId, String titleId) {
        TitleTemplate template = getTemplate(titleId);
        if (template == null) return false;
        PlayerTitle pt = getPlayerTitle(playerId);
        if (!pt.getOwnedTitleIds().contains(titleId)) return false;
        if (getPlayerLevel(playerId) < template.getRequiredLevel()) {
            throw new IllegalArgumentException("等级不足，需要达到 " + template.getRequiredLevel() + " 级");
        }
    ...
    /**
     * The level logic stores player levels in the shared "level" collection.
     * Reading the small document here keeps title unlocks server-authoritative
     * without coupling this logic module to the level implementation module.
     */
    public int getPlayerLevel(long playerId) {
        Document level = mongoTemplate.findById(playerId, Document.class, "level");
        if (level == null) return 1;
        Object value = level.get("level");
        return value instanceof Number number ? Math.max(1, number.intValue()) : 1;
    }

    public boolean claimTitle(long playerId, String titleId) {
        TitleTemplate template = getTemplate(titleId);
        if (template == null) throw new IllegalArgumentException("称号不存在");
        if (getPlayerLevel(playerId) < template.getRequiredLevel()) {
            throw new IllegalArgumentException("等级不足，需要达到 " + template.getRequiredLevel() + " 级");
        }
