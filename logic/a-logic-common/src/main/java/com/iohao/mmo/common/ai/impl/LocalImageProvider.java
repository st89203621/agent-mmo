...
public class LocalImageProvider implements AiImageProvider {
    ...
    private static final class WorkflowBuilder {
        ...
        private static int resolveSteps(AiProperties.Image.Local cfg, AiImageRequest req) {
            return req.getStepsOverride() > 0 ? req.getStepsOverride() : cfg.getSteps();
        }

        private static double resolveCfg(AiProperties.Image.Local cfg, AiImageRequest req) {
            return req.getCfgOverride() > 0 ? req.getCfgOverride() : cfg.getCfg();
        }

        private static String resolveNegative(AiProperties.Image.Local cfg, AiImageRequest req) {
            String v = req.getNegativePromptOverride();
            return (v == null || v.isBlank()) ? cfg.getNegativePrompt() : v;
        }
        ...
        private static JSONObject flux2(AiProperties.Image.Local cfg, AiImageRequest req,
            ...
            p.put("5", node("CLIPTextEncode", map("text", resolveNegative(cfg, req), "clip", ref("2", 0))));
            Latent latent = latentSource(p, cfg, req, uploadedImage, ref("3", 0));
            p.put("7", node("KSampler", map(
                    "seed", seed, "steps", resolveSteps(cfg, req), "cfg", resolveCfg(cfg, req),
        ...
        private static JSONObject sdxl(AiProperties.Image.Local cfg, AiImageRequest req,
            ...
            p.put("7", node("CLIPTextEncode", map("text", resolveNegative(cfg, req), "clip", ref("4", 1))));
