...
import com.iohao.mmo.common.ai.impl.LocalVideoProvider;
...
public class AiAutoConfiguration {
    ...
    /**
     * 视频生成目前仅支持 ComfyUI + LTX-Video。沿用 image 的 provider 切换语义：
     * 当 ai.image.provider=local 时启用 LocalVideoProvider，否则视频功能不可用（返回 null bean
     * 会导致注入失败，故始终返回 local 实例；在 image=volcengine 场景请避免调用动画接口）。
     */
    @Bean
    public AiVideoProvider aiVideoProvider(AiProperties props) {
        AiVideoProvider provider = new LocalVideoProvider(props.getVideo().getLocal());
        log.info("AiVideoProvider -> {}", provider.providerName());
        return provider;
    }
