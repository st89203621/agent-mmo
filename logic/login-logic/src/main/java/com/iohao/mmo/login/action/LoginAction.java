...
public class LoginAction implements ApplicationContextAware {
    ...
    public UserInfo loginVerify(LoginVerify loginVerify, FlowContext flowContext) {
        log.info("收到登录请求: username={}", loginVerify.username);
        ...
        log.warn("登录请求缺少必要参数: username={}", loginVerify.username);
    ...
    public RegisterResponse register(RegisterRequest registerRequest) {
        log.info("收到注册请求: username={}", registerRequest.username);

        String username = registerRequest.username;
        String password = registerRequest.password;

        if (username == null || username.isEmpty() || password == null || password.isEmpty()) {
            log.warn("注册请求缺少必要参数: username={}", username);
            throw new MsgException(GameCode.loginFailed);
        }
