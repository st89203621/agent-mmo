...
import com.iohao.mmo.pet.proto.DeployPetMessage;
...
public class PetAction {
    ...
    public List<PetMessage> listPet(FlowContext flowContext) {
        ...
        return messages(petService.ofPetBag(userId));
    }
    ...
    @ActionMethod(PetCmd.deployPet)
    public PetMessage deployPet(DeployPetMessage message, FlowContext flowContext) {
        Pet pet = petService.deploy(flowContext.getUserId(), message.petId);
        PetMessage result = PetMapper.ME.convert(pet);
        result.active = true;
        return result;
    }

    @ActionMethod(PetCmd.getContractPets)
    public List<PetMessage> getContractPets(FlowContext flowContext) {
        return messages(petService.contractPets(flowContext.getUserId()));
    }

    @ActionMethod(PetCmd.trainPet)
    public List<PetMessage> trainPet(String petId, FlowContext flowContext) {
        return messages(petService.trainContract(flowContext.getUserId(), petId));
    }

    private List<PetMessage> messages(PetBag bag) {
        List<PetMessage> result = PetMapper.ME.convertPets(bag.listPet());
        result.forEach(pet -> {
            pet.active = java.util.Objects.equals(pet.id, bag.getActivePetId());
            Pet source = bag.getPet(pet.id);
            if (source != null) { pet.level = Math.max(1, source.getLevel()); pet.tier = source.getTier(); }
        });
        return result;
    }
}
