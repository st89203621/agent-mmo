...
public class PetService {
    ...
    public synchronized PetBag contractPets(long userId) {
        PetBag bag = ofPetBag(userId);
        if (!bag.isStarterContractsGranted()) {
            for (int i = 0; i < 2; i++) {
                Pet pet = new Pet();
                pet.setId("contract_" + userId + "_" + i);
                pet.setPetTemplateId(i == 0 ? "contract_wolf" : "contract_fox");
                pet.setNickname(i == 0 ? "砾灵" : "灯灵");
                pet.setPetType(i == 0 ? "wolf" : "fox");
                pet.setElement(i == 0 ? "earth" : "light");
                pet.setLevel(1);
                pet.setTier(1);
                pet.setConstitution(5); pet.setPower(5); pet.setEndurance(5); pet.setAgile(5);
                pet.setPropertyPointNum(30); pet.setMaxSkill(4);
                bag.addPet(pet);
                if (i == 0 && bag.getActivePetId() == null) bag.setActivePetId(pet.getId());
            }
            bag.setStarterContractsGranted(true);
            save(bag);
        }
        return bag;
    }

    public synchronized PetBag trainContract(long userId, String petId) {
        PetBag bag = ofPetBag(userId);
        Pet pet = bag.getPet(petId);
        GameCode.petNotExist.assertNonNull(pet);
        int level = Math.max(1, pet.getLevel());
        GameCode.objNotFound.assertTrue(level < 30);
        shopService.consumeCraftCost(userId, 1200 + level * 150, "material_003", 2);
        pet.setLevel(level + 1);
        pet.setPower(pet.getPower() + 2);
        pet.setConstitution(pet.getConstitution() + 2);
        pet.setPropertyPointNum(pet.getPropertyPointNum() + 4);
        save(bag);
        return bag;
    }

    public synchronized Pet deploy(long userId, String petId) {
        GameCode.petNotExist.assertTrue(petId != null && !petId.isBlank());
        PetBag bag = ofPetBag(userId);
        Pet pet = bag.getPet(petId);
        GameCode.petNotExist.assertNonNull(pet);
        bag.setActivePetId(petId);
        save(bag);
        return pet;
    }
