    ...
    private void giveRewards(Quest quest, FlowContext flowContext) {
        ...
        for (QuestReward reward : quest.getRewards()) {
            switch (reward.getRewardType()) {
                ...
                case SKILL -> {
                    String skillTemplateId = reward.getItemId();
                    if (skillTemplateId == null || skillTemplateId.isBlank()) {
                        throw new IllegalArgumentException("技能奖励缺少技能模板");
                    }
                    skillService.grantSkillReward(userId, skillTemplateId);
                    log.info("任务奖励技能: userId={}, skill={}", userId, skillTemplateId);
                }
            }
        }
