...
public class ShopService {
    ...
    public void addCurrency(long userId, int gold, int diamond) {
        getPlayerCurrency(userId);
        Update update = new Update();
        if (gold > 0) update.inc("gold", gold);
        if (diamond > 0) update.inc("diamond", diamond);
        if (gold > 0 || diamond > 0) mongoTemplate.updateFirst(
                Query.query(Criteria.where("userId").is(userId)), update, PlayerCurrency.class);
    }

    public boolean trySpendGold(long userId, int amount) {
        if (amount <= 0) throw new IllegalArgumentException("扣款金额必须大于零");
        getPlayerCurrency(userId);
        return mongoTemplate.updateFirst(Query.query(Criteria.where("userId").is(userId).and("gold").gte(amount)),
                new Update().inc("gold", -amount), PlayerCurrency.class).getModifiedCount() == 1;
    }

    public void refundGold(long userId, int amount) {
        if (amount <= 0) throw new IllegalArgumentException("退款金额必须大于零");
        mongoTemplate.updateFirst(Query.query(Criteria.where("userId").is(userId)),
                new Update().inc("gold", amount), PlayerCurrency.class);
    }

    public void consumeCraftCost(long userId, int gold, String material, int quantity) {
        GameCode.quantityNotEnough.assertTrue(trySpendGold(userId, gold));
        boolean consumed = false;
        try {
            consumed = bagService.consumeMaterial(userId, material, quantity);
            GameCode.quantityNotEnough.assertTrue(consumed);
        } finally {
            if (!consumed) refundGold(userId, gold);
        }
    }

    public synchronized Map<String, Object> purchaseItem(long userId, String itemId, int quantity) {
        Map<String, Object> result = new HashMap<>();

        if (quantity <= 0 || quantity > 999) {
            result.put("success", false);
            result.put("message", "购买数量必须在1到999之间");
            return result;
        }
        ...
        boolean spent = mongoTemplate.updateFirst(Query.query(Criteria.where("userId").is(userId)
                        .and(item.getCurrency()).gte(totalPrice)),
                new Update().inc(item.getCurrency(), -totalPrice), PlayerCurrency.class).getModifiedCount() == 1;
        if (!spent) {
            result.put("success", false);
            result.put("message", "货币不足");
            return result;
        }
        currency = getPlayerCurrency(userId);
